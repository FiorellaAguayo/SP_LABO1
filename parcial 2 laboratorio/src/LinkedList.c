...
static Node* getNode(LinkedList *this, int nodeIndex)
{
	...
	{
		pNodo = this->pFirstNode; //inicializo pNodo a la dirección del primer nodo

		for (int i = 0; i < nodeIndex; i++)
		{
			pNodo = pNodo->pNextNode; // voy a iterar hata encontrar el indice pasado por parametro
		}
	}
...
LinkedList* ll_clone(LinkedList *this)
{
	...
	if (this != NULL && ll_subList(this, 0, ll_len(this)))
	{
		return cloneArray;
	}
	...
	if (this != NULL && pFunc != NULL && order >= 0 && order <= 1)
	{
		...
		for (int i = 0; i < len - 1; i++)
		{
			for (int j = i + 1; j < len; j++)
			{
				...
				else if (order == 0 && pFunc(elementoA, elementoB) == -1)
				{
					ll_set(this, j, elementoA);
					ll_set(this, i, elementoB);
				}
